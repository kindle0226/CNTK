...
if base_model_to_use == "AlexNet":
    ...
    start_train_conv_node_name = "conv3_1"
...
def clone_layers(base_model, from_node, to_node, train_starting_at_node=None):
    if train_starting_at_node == from_node:
        from_node = train_starting_at_node
        train_starting_at_node = None

    feature_node = find_by_name(base_model, from_node)
    last_conv_node = find_by_name(base_model, to_node)

    if train_starting_at_node == None:
        freeze_layers = combine([last_conv_node.owner]).clone(CloneMethod.freeze, {feature_node: placeholder()})
        return freeze_layers

    start_train_node = find_by_name(base_model, train_starting_at_node)
    freeze_layers = combine([start_train_node.owner]).clone(CloneMethod.freeze, {feature_node: placeholder()})
    train_layers = combine([last_conv_node.owner]).clone(CloneMethod.clone, {start_train_node: placeholder()})

    conv_layers = Sequential([freeze_layers, train_layers])
    return conv_layers

def clone_rpn(rpn_model, conv_input_node_name, roi_input_node_name,
              rois_node_name, losses_node_name, freeze = True):
    conv_input_node = find_by_name(rpn_model, conv_input_node_name)
    roi_input_node = find_by_name(rpn_model, roi_input_node_name)
    rois_node = find_by_name(rpn_model, rois_node_name)
    losses_node = find_by_name(rpn_model, losses_node_name)

    rpn_layers = combine([rois_node.owner, losses_node.owner])\
        .clone(CloneMethod.freeze if freeze else CloneMethod.clone,
               {conv_input_node: placeholder(), roi_input_node: placeholder()})
    return rpn_layers
...
def faster_rcnn_predictor(features, gt_boxes):
    ...
    conv_layers = clone_layers(base_model, feature_node_name, last_conv_node_name, train_starting_at_node=start_train_conv_node_name)
    # TODO: reset to CloneMethod.clone. Setting to freeze for now to try learning rates
    fc_layers = clone_layers(base_model, pool_node_name, last_hidden_node_name) # , train_starting_at_node=pool_node_name
...
def train_model(image_input, roi_input, predictions, loss, pred_error, lr_schedule, mm_schedule, l2_reg_weight, epochs_to_train):
    ...
    log_number_of_parameters(predictions)
...
def train_faster_rcnn_e2e(debug_output=False):
    ...
    train_model(image_input, roi_input, predictions, loss, pred_error, lr_schedule, mm_schedule, l2_reg_weight, epochs_to_train=max_epochs)
    return loss
...
def train_faster_rcnn_alternating(debug_output=False):
    ...
    if True:
        ...
        # train
        train_model(image_input, roi_input, rpn_losses, rpn_losses, rpn_losses,
    ...
    if True:
        ...
        # conv_layers
        conv_layers = clone_layers(base_model, feature_node_name, last_conv_node_name,
                                   train_starting_at_node=start_train_conv_node_name)
        conv_out = conv_layers(image_input)

        # RPN
        rpn = clone_rpn(stage1_rpn_network,
                        conv_input_node_name=last_conv_node_name,
                        roi_input_node_name="roi_input",
                        rois_node_name="rpn_rois",
                        losses_node_name="rpn_losses")
        rpn_net = rpn(conv_out, roi_input)
        rpn_rois = rpn_net.outputs[0]
        rpn_losses = rpn_net.outputs[1] # requires for training rpn in stage 2
        ...
        train_model(image_input, roi_input, detection_losses, detection_losses, detection_losses,
    ...
    # TODO: return stage 2 model
    stage2_frcn_network = stage1_frcn_network
